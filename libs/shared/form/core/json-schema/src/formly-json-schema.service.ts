...
export class FormlyJsonschema {
  ...
  private _toFieldConfig(
    ...
    if (key && options.strict) {
      this.addValidator(
        field,
        'type',
        (c: AbstractControl, f: FormlyFieldConfig) => {
    ...
    switch (field.type) {
      ...
      case 'integer': {
        field.parsers = [
          (v: string | number) => (isEmpty(v) ? undefined : Number(v)),
        ];
            ...
            ({ value }: AbstractControl) => {
              ...
              return (
                Math.round(value * multiplier) %
                  Math.round(schema.multipleOf * multiplier) ===
                0
              );
            }
          );
          ...
          if (isRequired || propDeps[property]) {
            f.expressions = {
              ...
              'props.required': (f) => {
                let parent = f.parent;
                const model =
                  f.fieldGroup && f.key != null ? parent.model : f.model;
                while (parent.key == null && parent.parent) {
                  parent = parent.parent;
                }

                const required =
                  parent && parent.props ? parent.props.required : false;
                if (!model && !required) {
                  return false;
                }
                ...
                return (
                  propDeps[property] &&
                  f.model &&
                  propDeps[property].some((k) => !isEmpty(f.model[k]))
                );
              },
            };
          }
            ...
            if (
              ...
              oneOfSchema.every(
                (o) =>
                  ...
                  isConst(o.properties[property])
              )
            ) {
              oneOfSchema.forEach((oneOfSchemaItem) => {
                const { [property]: constSchema, ...properties } =
                  oneOfSchemaItem.properties;
                field.fieldGroup.push({
                  ...this._toFieldConfig(
                    { ...oneOfSchemaItem, properties },
                    { ...options, resetOnHide: true }
                  ),
                  expressions: {
                    hide: (f) =>
                      !f.model ||
                      getConstValue(constSchema as JSONSchema7) !==
                        f.model[property],
                  },
                });
              });
        ...
        if (!this.isEnum(schema)) {
          field.fieldArray = (root: FormlyFieldConfig) => {
            if (!Array.isArray(schema.items)) {
              ...
              const f = schema.items
                ? this._toFieldConfig(<JSONSchema7>schema.items, options)
                : {};
              if (f.props) {
    ...
    if (schema.hasOwnProperty('const')) {
      ...
      this.addValidator(
        field,
        'const',
        ({ value }: AbstractControl) => value === schema.const
      );
      if (!field.type) {
      ...
      fieldGroup: [
        {
          ...
          hooks: {
            onInit: (f) =>
              f.formControl.valueChanges.pipe(
                tap(() => f.options.detectChanges(f.parent))
              ),
          },
        },
        {
              ...
              hide: (f, forceUpdate?: boolean) => {
                ...
                if (control.value === -1 || forceUpdate) {
                  let value = f.parent.fieldGroup
                    .map(
                      (f, i) =>
                        [f, i, this.isFieldValid(f, i, schemas, options)] as [
                          FormlyFieldConfig,
                          number,
                          boolean
                        ]
                  ...
                  if (mode === 'anyOf') {
                    const definedValue = value.filter((i) =>
                      totalMatchedFields(f.parent.fieldGroup[i])
                    );
                    value =
                      definedValue.length > 0 ? definedValue : [value[0] || 0];
                  }
  ...
  private resolveDefinition(
    ...
    return {
      ...definition,
      ...['title', 'description', 'default', 'widget'].reduce(
        (annotation, p) => {
  ...
  private addValidator(
    ...
    name: string,
    validator: FormlyFieldConfig['validators']
  ) {
  ...
  private isFieldValid(
    root: FormlyFieldConfig & {
      _schemasFields?: { [key: number]: FormlyFieldConfig };
    },
    i: number,
    schemas: JSONSchema7[],
    options: IOptions
