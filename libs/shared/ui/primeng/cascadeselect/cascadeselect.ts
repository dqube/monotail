...
export class CascadeSelectSub implements OnInit {
  ...
  constructor(
    @Inject(forwardRef(() => CascadeSelect)) cascadeSelect: CascadeSelect,
  ...
  getOptionGroupChildren(optionGroup: string | string[]) {
    return ObjectUtils.resolveFieldData(
      optionGroup,
      (this.optionGroupChildren as any)[this.level]
    );
  }
  ...
  getOptionLabelToRender(option: string | string[]) {
    return this.isOptionGroup(option)
      ...
      <ng-container *ngIf="filled && !disabled && showClear">
        <TimesIcon
          *ngIf="!clearIconTemplate"
          [styleClass]="'p-cascadeselect-clear-icon'"
          (click)="clear($event)"
        />
        <span
          *ngIf="clearIconTemplate"
      ...
      >
        <ChevronDownIcon
          *ngIf="!triggerIconTemplate"
          [styleClass]="'p-cascadeselect-trigger-icon'"
        />
...
export class CascadeSelect implements OnInit, AfterContentInit {
   ...
   * @group Emits
   */
  @Output() onShow: EventEmitter<CascadeSelectShowEvent> =
    new EventEmitter<CascadeSelectShowEvent>();
  /**
   * Callback to invoke when the overlay is hidden.
   * @param {CascadeSelectHideEvent} - custom event.
   * @group Emits
   */
  @Output() onHide: EventEmitter<CascadeSelectHideEvent> =
    new EventEmitter<CascadeSelectHideEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onBeforeShow: EventEmitter<CascadeSelectBeforeShowEvent> =
    new EventEmitter<CascadeSelectBeforeShowEvent>();
  /**
   * Callback to invoke before overlay is hidden.
   * @group Emits
   */
  @Output() onBeforeHide: EventEmitter<CascadeSelectBeforeHideEvent> =
    new EventEmitter<CascadeSelectBeforeHideEvent>();
  /**
  ...
  getOptionGroupChildren(optionGroup: string | object, level: number) {
    return ObjectUtils.resolveFieldData(
      optionGroup,
      (this.optionGroupChildren as string)[level]
    );
  }
  ...
  updateFilledState() {
    this.filled = !(
      this.selectionPath == null || this.selectionPath.length == 0
    );
  }

  findModelOptionInGroup(
    ...
    if (this.isOptionGroup(option, level)) {
      ...
      ObjectUtils.equals(this.value, this.getOptionValue(option), this.dataKey)
    ) {
      return [option];
    }
