...
export class Slider implements OnDestroy, ControlValueAccessor {
   ...
   * @group Emits
   */
  @Output() onChange: EventEmitter<SliderChangeEvent> =
    new EventEmitter<SliderChangeEvent>();
  /**
   * Callback to invoke when slide ended.
   * @param {SliderSlideEndEvent} event - Custom slide end event.
   * @group Emits
   */
  @Output() onSlideEnd: EventEmitter<SliderSlideEndEvent> =
    new EventEmitter<SliderSlideEndEvent>();
  ...
  onTouchStart(event: TouchEvent, index?: number) {
    ...
    this.startHandleValue = this.range
      ? this.handleValues[index as number]
      : this.handleValue;
    this.dragging = true;
    if (this.range && this.handleValues && this.handleValues[0] === this.max) {
      this.handleIndex = 0;
  ...
  onTouchMove(event: TouchEvent) {
    ...
    if (this.orientation === 'horizontal') {
      ...
      handleValue =
        Math.floor(
          (((this.starty as number) - parseInt((touchobj as any).clientY, 10)) *
            100) /
            (this.barHeight as number)
        ) + this.startHandleValue;
    }
  ...
  get rangeStartLeft() {
    if (!this.isVertical())
      return this.handleValues[0] > 100
        ? 100 + '%'
        : this.handleValues[0] + '%';
    return null;
  }
  ...
  updateHandleValue(): void {
    if (this.range) {
      this.handleValues[0] =
        (((this.values as number[])[0] < this.min
          ? 0
          : (this.values as number[])[0] - this.min) *
          100) /
        (this.max - this.min);
      this.handleValues[1] =
        (((this.values as number[])[1] > this.max
          ? 100
          : (this.values as number[])[1] - this.min) *
          100) /
        (this.max - this.min);
    } else {
      if ((this.value as number) < this.min) this.handleValue = 0;
        ...
        this.handleValue =
          (((this.value as number) - this.min) * 100) / (this.max - this.min);
    }
  ...
  updateValue(val: number, event?: Event): void {
    if (this.range) {
      ...
      (this.values as number[])[this.handleIndex] =
