  ...
  template: `
    ...
    >
      ...
      >
        <i
          *ngIf="loadingIcon"
          [class]="'p-datatable-loading-icon ' + loadingIcon"
        ></i>
  ...
  implements OnInit, AfterViewInit, AfterContentInit, BlockableUI, OnChanges
{
  ...
  set responsive(val: boolean | undefined | null) {
    ...
    console.warn(
      'responsive propery is deprecated as table is always responsive with scrollable behavior.'
    );
  }
  ...
  @Output() selectAllChange: EventEmitter<TableSelectAllChangeEvent> =
    new EventEmitter<TableSelectAllChangeEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onRowSelect: EventEmitter<TableRowSelectEvent> =
    new EventEmitter<TableRowSelectEvent>();
  /**
   * Callback to invoke when a row is unselected.
   * @param {TableRowUnSelectEvent} event - custom unselect event.
   * @group Emits
   */
  @Output() onRowUnselect: EventEmitter<TableRowUnSelectEvent> =
    new EventEmitter<TableRowUnSelectEvent>();
  /**
   * Callback to invoke when pagination occurs.
   * @param {TablePageEvent} event - custom pagination event.
   * @group Emits
   */
  @Output() onPage: EventEmitter<TablePageEvent> =
    new EventEmitter<TablePageEvent>();
  /**
   * Callback to invoke when a column gets sorted.
   * @param {Object} object - sort meta.
   * @group Emits
   */
  @Output() onSort: EventEmitter<{ multisortmeta: SortMeta[] } | any> =
    new EventEmitter<{ multisortmeta: SortMeta[] } | any>();
  /**
   * Callback to invoke when data is filtered.
   * @param {TableFilterEvent} event - custom filtering event.
   * @group Emits
   */
  @Output() onFilter: EventEmitter<TableFilterEvent> =
    new EventEmitter<TableFilterEvent>();
  /**
   * Callback to invoke when paging, sorting or filtering happens in lazy mode.
   * @param {TableLazyLoadEvent} event - custom lazy loading event.
   * @group Emits
   */
  @Output() onLazyLoad: EventEmitter<TableLazyLoadEvent> =
   ...
   * @group Emits
   */
  @Output() onRowExpand: EventEmitter<TableRowExpandEvent> =
    new EventEmitter<TableRowExpandEvent>();
  /**
   * Callback to invoke when a row is collapsed.
   * @param {TableRowCollapseEvent} event - custom row collapse event.
   * @group Emits
   */
  @Output() onRowCollapse: EventEmitter<TableRowCollapseEvent> =
    new EventEmitter<TableRowCollapseEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onColResize: EventEmitter<TableColResizeEvent> =
    new EventEmitter<TableColResizeEvent>();
  /**
   * Callback to invoke when a column is reordered.
   * @param {TableColumnReorderEvent} event - custom column reorder event.
   * @group Emits
   */
  @Output() onColReorder: EventEmitter<TableColumnReorderEvent> =
    new EventEmitter<TableColumnReorderEvent>();
  /**
   * Callback to invoke when a row is reordered.
   * @param {TableRowReorderEvent} event - custom row reorder event.
   * @group Emits
   */
  @Output() onRowReorder: EventEmitter<TableRowReorderEvent> =
    new EventEmitter<TableRowReorderEvent>();
  /**
   * Callback to invoke when a cell switches to edit mode.
   * @param {TableEditInitEvent} event - custom edit init event.
   * @group Emits
   */
  @Output() onEditInit: EventEmitter<TableEditInitEvent> =
    new EventEmitter<TableEditInitEvent>();
  /**
   * Callback to invoke when cell edit is completed.
   * @param {TableEditCompleteEvent} event - custom edit complete event.
   * @group Emits
   */
  @Output() onEditComplete: EventEmitter<TableEditCompleteEvent> =
    new EventEmitter<TableEditCompleteEvent>();
  /**
   * Callback to invoke when cell edit is cancelled with escape key.
   * @param {TableEditCancelEvent} event - custom edit cancel event.
   * @group Emits
   */
  @Output() onEditCancel: EventEmitter<TableEditCancelEvent> =
    new EventEmitter<TableEditCancelEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onStateSave: EventEmitter<TableState> =
    new EventEmitter<TableState>();
  /**
   * Callback to invoke table state is restored.
   * @param {TableState} object - table state.
   * @group Emits
   */
  @Output() onStateRestore: EventEmitter<TableState> =
    new EventEmitter<TableState>();
  ...
  sort(event: any) {
    ...
    if (this.sortMode === 'multiple') {
      let metaKey =
        (<KeyboardEvent>originalEvent).metaKey ||
        (<KeyboardEvent>originalEvent).ctrlKey;
      let sortMeta = this.getSortMeta(<string>event.field);

      if (sortMeta) {
        if (!metaKey) {
          this._multiSortMeta = [
            { field: <string>event.field, order: sortMeta.order * -1 },
          ];
          ...
          if (selected && metaKey) {
            if (this.isSingleSelectionMode()) {
              ...
              this._selection = this.selection.filter(
                (val: any, i: number) => i != selectionIndex
              );
        ...
        for (let i = 0; i < this.value.length; i++) {
          ...
          if (
            ...
            for (let j = 0; j < globalFilterFieldsArray.length; j++) {
              ...
              globalMatch = (<any>this.filterService).filters[
  ...
  onColumnResize(event: any) {
    ...
    (<ElementRef>this.resizeHelperViewChild).nativeElement.style.display =
      'block';
  }
  ...
  onColumnResizeEnd() {
    let delta =
      ...
      <number>this.lastResizerHelperX;
    let columnWidth = this.resizeColumnElement.offsetWidth;
    let newColumnWidth = columnWidth + delta;
    let minWidth =
      this.resizeColumnElement.style.minWidth.replace(/[^\d.]/g, '') || 15;
  ...
  resizeTableCells(newColumnWidth: number, nextColumnWidth: number | null) {
    ...
    widths.forEach((width, index) => {
      ...
      innerHTML += `
                #${this.id}-table > .p-datatable-thead > tr > th:nth-child(${
        index + 1
      }),
                #${this.id}-table > .p-datatable-tbody > tr > td:nth-child(${
        index + 1
      }),
                #${this.id}-table > .p-datatable-tfoot > tr > td:nth-child(${
        index + 1
      }) {
                    ${style}
                }
    ...
    if (this.reorderableColumns && this.draggedColumn && dropHeader) {
      ...
      if (this.draggedColumn != dropHeader) {
        ...
        (<ElementRef>this.reorderIndicatorUpViewChild).nativeElement.style.top =
          ...
          containerOffset.top -
          (<number>this.reorderIconHeight - 1) +
          'px';
        (<ElementRef>(
        ...
        if (event.pageX > columnCenter) {
          (<ElementRef>(
            ...
            Math.ceil(<number>this.reorderIconWidth / 2) +
            'px';
          (<ElementRef>(
            this.reorderIndicatorDownViewChild
          )).nativeElement.style.left =
...
export class TableBody implements AfterViewInit, OnDestroy {
  ...
  getRowIndex(rowIndex: number) {
    const index = this.dt.paginator
      ? <number>this.dt.first + rowIndex
      : rowIndex;
...
export class SortIcon implements OnInit, OnDestroy {
  ...
  updateSortState() {
    if (this.dt.sortMode === 'single') {
      this.sortOrder = this.dt.isSorted(<string>this.field)
        ? this.dt.sortOrder
        : 0;
    ...
    if (this.dt._selectAll !== null) {
      ...
      const selectableVal = this.dt.rowSelectable
        ? val.filter((data: any, index: number) =>
            this.dt.rowSelectable({ data, index })
          )
        : val;
...
export class ReorderableRow implements AfterViewInit {
  ...
  bindEvents() {
    this.zone.runOutsideAngular(() => {
      ...
      this.dragEndListener = this.renderer.listen(
        this.el.nativeElement,
        'dragend',
        ...
        <ng-template #menu>
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <ng-template
                    *ngTemplateOutlet="removeRuleIconTemplate"
                  ></ng-template>
                </button>
              </div>
            </div>
          ...
          <div class="p-column-filter-add-rule" *ngIf="isShowAddConstraint">
            ...
            >
              ...
              <ng-template
                *ngTemplateOutlet="addRuleIconTemplate"
              ></ng-template>
            </button>
          </div>
...
export class ColumnFilter implements AfterContentInit {
  ...
  generateMatchModeOptions() {
    this.matchModes =
      ...
      (this.config as any).filterMatchModeOptions[this.type]?.map(
        (key: any) => {
  ...
  isRowMatchModeSelected(matchMode: string) {
    return (
      (<FilterMetadata>this.dt.filters[<string>this.field]).matchMode ===
      matchMode
    );
  }
    ...
    switch (event.key) {
      ...
      case 'ArrowDown':
        if (this.overlayVisible) {
          let focusable = DomHandler.getFocusableElements(
            <HTMLElement>this.overlay
          );
          if (focusable) {
            focusable[0].focus();
          }
  ...
  hasRowFilter() {
    return (
      ...
      !this.dt.isFilterBlank(
        (<FilterMetadata>this.dt.filters[<string>this.field]).value
      )
    );
  }
