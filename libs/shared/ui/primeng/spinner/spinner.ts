...
@Component({
  ...
  template: `
    <span class="ui-spinner ui-widget ui-corner-all">
      ...
      >
        <span
          class="ui-spinner-button-icon pi pi-caret-down ui-clickable"
        ></span>
      </button>
    </span>
  `,
...
export class Spinner implements OnInit, ControlValueAccessor {
  ...
  ngOnInit() {
    if (this.formatInput) {
      ...
      this.thousandRegExp = new RegExp(
        `[${this.thousandSeparator || this.localeThousandSeparator}]`,
        'gim'
      );
      ...
      if (
        ...
        console.warn(
          'thousandSeparator and decimalSeparator cannot have the same value.'
        );
      }
    }
  }
  ...
  parseValue(val: string): number {
    ...
    if (val.trim() === '') {
      ...
      if (precision) {
        val = this.formatInput
          ? val.replace(
              this.decimalSeparator || this.localeDecimalSeparator,
              '.'
            )
          : val.replace(',', '.');
      ...
      if (this.formatInput) {
        ...
        if (this.decimalSeparator && this.thousandSeparator) {
          ...
          if (precision && value[1]) {
            value[1] =
              (this.decimalSeparator || this.localeDecimalSeparator) + value[1];
          }

          if (this.thousandSeparator && value[0].length > 3) {
            value[0] = value[0].replace(
              new RegExp(`[${this.localeThousandSeparator}]`, 'gim'),
              this.thousandSeparator
            );
          }
