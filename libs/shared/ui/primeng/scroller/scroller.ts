    ...
    <ng-container *ngIf="!_disabled; else disabledContainer">
      ...
      >
        ...
        <ng-template #buildInContent>
          <div
            #content
            class="p-scroller-content"
            [ngClass]="{ 'p-scroller-loading': d_loading }"
                ...
                *ngTemplateOutlet="
                  ...
                  context: {
                    options: getLoaderOptions(
                      index,
                      both && { numCols: _numItemsInViewport.cols }
                    )
                  }
                "
              ></ng-container>
            ...
            <ng-container *ngIf="loaderIconTemplate; else buildInLoaderIcon">
              <ng-container
                *ngTemplateOutlet="
                  loaderIconTemplate;
                  context: {
                    options: { styleClass: 'p-scroller-loading-icon' }
                  }
                "
              ></ng-container>
            </ng-container>
  ...
  implements OnInit, AfterContentInit, AfterViewChecked, OnDestroy
{
  ...
  @Output() onLazyLoad: EventEmitter<ScrollerLazyLoadEvent> =
    new EventEmitter<ScrollerLazyLoadEvent>();
  /**
  ...
  @Output() onScroll: EventEmitter<ScrollerScrollEvent> =
    new EventEmitter<ScrollerScrollEvent>();
  /**
  ...
  viewInit() {
    if (isPlatformBrowser(this.platformId)) {
      if (DomHandler.isVisible(this.elementViewChild?.nativeElement)) {
        ...
        this.defaultWidth = DomHandler.getWidth(
          this.elementViewChild?.nativeElement
        );
        this.defaultHeight = DomHandler.getHeight(
          this.elementViewChild?.nativeElement
        );
  ...
  getPageByFirst() {
    return Math.floor(
      (this.first + this.d_numToleratedItems * 4) / (this._step || 1)
    );
  }
  ...
  scrollToIndex(index: number, behavior: ScrollBehavior = 'auto') {
    ...
    const calculateFirst = (_index = 0, _numT: number) =>
    ...
    if (this.both) {
      newFirst = {
        rows: calculateFirst((<any>index)[0], numToleratedItems[0]),
        cols: calculateFirst((<any>index)[1], numToleratedItems[1]),
      };
      scrollTo(
        calculateCoord(
          newFirst.cols,
          (<number[]>this._itemSize)[1],
          contentPos.left
        ),
        calculateCoord(
          newFirst.rows,
          (<number[]>this._itemSize)[0],
          contentPos.top
        )
      );
    } else {
      ...
      this.horizontal
        ? scrollTo(
            calculateCoord(newFirst, <number>this._itemSize, contentPos.left),
            0
          )
        : scrollTo(
            0,
            calculateCoord(newFirst, <number>this._itemSize, contentPos.top)
          );
    }
  ...
  calculateNumItems() {
    ...
    const numItemsInViewport: any = this.both
      ...
      : calculateNumItemsInViewport(
          this.horizontal ? contentWidth : contentHeight,
          <number>this._itemSize
        );
    ...
    if (this._autoSize && !this.d_loading) {
      Promise.resolve().then(() => {
        if (this.contentEl) {
          ...
          const [width, height] = [
            DomHandler.getWidth(
              (<ElementRef>this.elementViewChild).nativeElement
            ),
  ...
  getContentPosition() {
    if (this.contentEl) {
      ...
      const top =
        parseFloat(style.paddingTop) + Math.max(parseFloat(style.top) || 0, 0);
      const bottom =
  ...
  setSize() {
    if (this.elementViewChild?.nativeElement) {
      const parentElement =
        this.elementViewChild.nativeElement.parentElement.parentElement;
      const width =
        this._scrollWidth ||
        `${
      ...
      const setProp = (_name: string, _value: any) =>
        ((<ElementRef>this.elementViewChild).nativeElement.style[_name] =
          _value);
  ...
  setContentPosition(pos: any) {
    if (this.contentEl && !this._appendOnly) {
      ...
      const calculateTranslateVal = (_first: number, _size: number) =>
        _first * _size;
        ...
        const lazyLoadState = {
          first: this._step
            ? Math.min(
                ...
                (<any[]>this.items).length - this._step
              )
            : first,
          last: Math.min(
            this._step ? (this.getPageByFirst() + 1) * this._step : last,
            (<any[]>this.items).length
          ),
        };
        const isLazyStateChanged =
          this.lazyLoadState.first !== lazyLoadState.first ||
  ...
  onContainerScroll(event: Event) {
    ...
    if (this._delay && this.isPageChanged) {
      ...
      if (!this.d_loading && this.showLoader) {
        ...
        const changed =
          isRangeChanged || (this._step ? this.isPageChanged : false);
    ...
    if (isPlatformBrowser(this.platformId)) {
      if (!this.windowResizeListener) {
        this.zone.runOutsideAngular(() => {
          ...
          const event = DomHandler.isTouchDevice()
            ? 'orientationchange'
            : 'resize';
  ...
  handleEvents(name: string, params: any) {
    //@ts-ignore
    return this.options && (<any>this.options)[name]
      ? (<any>this.options)[name](params)
      : this[name].emit(params);
  }

  getContentOptions() {
    return {
      contentStyleClass: `p-scroller-content ${
        this.d_loading ? 'p-scroller-loading' : ''
      }`,
      items: this.loadedItems,
      getItemOptions: (index: number) => this.getOptions(index),
      loading: this.d_loading,
      getLoaderOptions: (index: number, options?: any) =>
        this.getLoaderOptions(index, options),
