...
describe('Calendar', () => {
  ...
  it('should change date format', () => {
    ...
    if (calendar.currentMonth < 9)
      expect(calendar.inputFieldValue).toEqual(
        '08/0' + (calendar.currentMonth + 1) + '/' + calendar.currentYear
      );
    else
      expect(calendar.inputFieldValue).toEqual(
  ...
  it('should show time', () => {
    ...
    expect(
      hourPicker.queryAll(By.css('span'))[0].nativeElement.textContent
    ).not.toEqual(defaultHour.toString());
    expect(
      minutePicker.queryAll(By.css('span'))[0].nativeElement.textContent
  ...
  it('should use touchUI', fakeAsync(() => {
    ...
    if (calendar.currentMonth < 9)
      expect(calendar.inputFieldValue).toEqual(
        '0' + (calendar.currentMonth + 1) + '/01/' + calendar.currentYear
      );
    else
      expect(calendar.inputFieldValue).toEqual(
  ...
  it('should show seconds', () => {
    ...
    expect(
      secondPicker.queryAll(By.css('span'))[0].nativeElement.textContent
    ).toEqual('21');
  });
  ...
  it('should highlight selected range in month view', () => {
    ...
    const firstMonthEl = containerEl.queryAll(
      By.css('span.p-monthpicker-month:not(.p-disabled)')
    )[0].nativeElement;
    const secondMonthEl = containerEl.queryAll(
      By.css('span.p-monthpicker-month:not(.p-disabled)')
    )[1].nativeElement;
    const thirdMonthEl = containerEl.queryAll(
      By.css('span.p-monthpicker-month:not(.p-disabled)')
    )[2].nativeElement;
  ...
  it('should show disabled months in month range selection mode', () => {
    ...
    [0, 1, 2, 7, 8, 9, 10, 11].forEach((m) => {
      expect(monthEl(m).classList)
        .withContext(`${m} month must be disabled`)
        .toContain('p-disabled');
    });

    [3, 4, 5, 6].forEach((m) => {
      expect(monthEl(m).classList)
        .withContext(`${m} month must be enabled`)
