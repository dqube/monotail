...
export class Calendar implements OnInit, OnDestroy, ControlValueAccessor {
   ...
   * @group Emits
   */
  @Output() onClose: EventEmitter<AnimationEvent> =
   ...
   * @group Emits
   */
  @Output() onMonthChange: EventEmitter<CalendarMonthChangeEvent> =
    new EventEmitter<CalendarMonthChangeEvent>();
  /**
   * Callback to invoke when a year is changed using the navigators.
   * @param {CalendarYearChangeEvent} event - custom year change event.
   * @group Emits
   */
  @Output() onYearChange: EventEmitter<CalendarYearChangeEvent> =
    new EventEmitter<CalendarYearChangeEvent>();
  /**
  ...
  incrementYear() {
    ...
    if (
      this.yearNavigator &&
      this.currentYear > _yearOptions[_yearOptions.length - 1]
    ) {
      let difference = _yearOptions[_yearOptions.length - 1] - _yearOptions[0];
      this.populateYearOptions(
        _yearOptions[0] + difference,
  ...
  updateModel(value: any) {
    ...
    if (this.dataType == 'date') {
      ...
      if (this.isSingleSelection()) {
        ...
        if (this.value) {
          stringArrValue = this.value.map((date: Date) =>
  ...
  incrementHour(event: any) {
    ...
    if (
      this.validateTime(
        newHour,
        <any>this.currentMinute,
        <any>this.currentSecond,
        <any>newPM
      )
    ) {
  ...
  appendOverlay() {
    if (this.appendTo) {
      if (this.appendTo === 'body')
        this.document.body.appendChild(<HTMLElement>this.overlay);
    ...
    for (iFormat = 0; iFormat < format.length; iFormat++) {
      if (literal) {
        ...
        switch (format.charAt(iFormat)) {
          ...
          case '!':
            date = new Date(
              (getNumber('!') - <number>this.ticksTo1970) / 10000
            );
            year = date.getFullYear();
            month = date.getMonth() + 1;
            day = date.getDate();
  ...
  updateFilledState() {
    this.filled = (this.inputFieldValue &&
      this.inputFieldValue != '') as boolean;
  }
