...
export class TieredMenuSub implements OnDestroy {
    ...
    switch (event.key) {
      ...
      case 'ArrowRight':
        if (item.items) {
          ...
          setTimeout(() => {
            (
              listItem?.children[1].children[0].children[0]
  ...
  positionSubmenu() {
    ...
    if (sublist) {
      ...
      const sublistWidth = sublist.offsetParent
        ? sublist.offsetWidth
    ...
    if (event.key === 'ArrowLeft') {
      ...
      if (event.target) {
        ...
        if (
          ...
          (
            parent.parentElement.parentElement.children[0] as HTMLElement
          ).focus();
        }
      }
    }
