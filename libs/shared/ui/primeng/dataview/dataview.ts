    ...
    >
      <div class="p-dataview-loading" *ngIf="loading">
        <div class="p-dataview-loading-overlay p-component-overlay">
          <i
            *ngIf="loadingIcon"
            [class]="'p-dataview-loading-icon pi-spin ' + loadingIcon"
          ></i>
          <ng-container *ngIf="!loadingIcon">
            <SpinnerIcon
              *ngIf="!loadingIconTemplate"
              [spin]="true"
              [styleClass]="'p-dataview-loading-icon'"
            />
      ...
      <div class="p-dataview-content">
        <div
          class="p-grid p-nogutter grid-nogutter grid"
          [ngClass]="gridStyleClass"
        >
          <ng-template
            ngFor
            let-rowData
            ...
            [ngForOf]="
              paginator
                ? (filteredValue || value
                  | slice : (lazy ? 0 : first) : (lazy ? 0 : first) + rows)
                : filteredValue || value
            "
...
export class DataView
  implements OnInit, AfterContentInit, OnDestroy, BlockableUI, OnChanges
{
  /**
   ...
   * @group Emits
   */
  @Output() onLazyLoad: EventEmitter<DataViewLazyLoadEvent> =
    new EventEmitter<DataViewLazyLoadEvent>();
  /**
   * Callback to invoke when pagination occurs.
   * @param {DataViewPageEvent} event - custom page event.
   * @group Emits
   */
  @Output() onPage: EventEmitter<DataViewPageEvent> =
    new EventEmitter<DataViewPageEvent>();
  /**
   * Callback to invoke when sorting occurs.
   * @group Emits
   */
  @Output() onSort: EventEmitter<DataViewSortEvent> =
    new EventEmitter<DataViewSortEvent>();
  /**
   * Callback to invoke when changing layout.
   * @param {DataViewLayoutChangeEvent} event - custom layout change event.
   * @group Emits
   */
  @Output() onChangeLayout: EventEmitter<DataViewLayoutChangeEvent> =
    new EventEmitter<DataViewLayoutChangeEvent>();
  ...
  filter(filter: string, filterMatchMode: string = 'contains') {
    ...
    if (this.value && this.value.length) {
      ...
      this.filteredValue = this.filterService.filter(
        this.value,
        searchFields,
        filter,
        filterMatchMode,
...
@Component({
  ...
  template: `
    <div
      [ngClass]="'p-dataview-layout-options p-selectbutton p-buttonset'"
      [ngStyle]="style"
      [class]="styleClass"
    >
      <button
      ...
      >
        ...
        <ng-template
          *ngTemplateOutlet="dv.listIconTemplate"
        ></ng-template></button
      ><button
        type="button"
        class="p-button p-button-icon-only"
