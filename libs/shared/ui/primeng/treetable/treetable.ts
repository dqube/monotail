  ...
  implements AfterContentInit, OnInit, OnDestroy, BlockableUI, OnChanges
{
  ...
  set virtualRowHeight(val: number) {
    ...
    console.warn(
      'The virtualRowHeight property is deprecated, use virtualScrollItemSize property instead.'
    );
  }
  /**
   ...
   * @group Emits
   */
  @Output() selectionChange: EventEmitter<TreeTableNode | null> =
    new EventEmitter<TreeTableNode | null>();
  /**
   * Callback to invoke on context menu selection change.
   * @param {TreeTableNode} object - node instance.
   * @group Emits
   */
  @Output() contextMenuSelectionChange: EventEmitter<TreeTableNode> =
   ...
   * @group Emits
   */
  @Output() onFilter: EventEmitter<TreeTableFilterEvent> =
    new EventEmitter<TreeTableFilterEvent>();
  /**
   * Callback to invoke when a node is expanded.
   * @param {TreeTableNode} object - node instance.
   * @group Emits
   */
  @Output() onNodeExpand: EventEmitter<TreeTableNode> =
    new EventEmitter<TreeTableNode>();
  /**
   ...
   * @group Emits
   */
  @Output() onNodeCollapse: EventEmitter<TreeTableNodeCollapseEvent> =
    new EventEmitter<TreeTableNodeCollapseEvent>();
  /**
   * Callback to invoke when pagination occurs.
   * @param {TreeTablePaginatorState} object - paginator state.
   * @group Emits
   */
  @Output() onPage: EventEmitter<TreeTablePaginatorState> =
    new EventEmitter<TreeTablePaginatorState>();
  /**
   ...
   * @group Emits
   */
  @Output() onLazyLoad: EventEmitter<TreeTableLazyLoadEvent> =
    new EventEmitter<TreeTableLazyLoadEvent>();
  /**
   * An event emitter to invoke on custom sorting, refer to sorting section for details.
   * @param {TreeTableSortEvent} event - custom sort event.
   * @group Emits
   */
  @Output() sortFunction: EventEmitter<TreeTableSortEvent> =
    new EventEmitter<TreeTableSortEvent>();
  /**
   * Callback to invoke when a column is resized.
   * @param {TreeTableColResizeEvent} event - custom column resize event.
   * @group Emits
   */
  @Output() onColResize: EventEmitter<TreeTableColResizeEvent> =
    new EventEmitter<TreeTableColResizeEvent>();
  /**
   * Callback to invoke when a column is reordered.
   * @param {TreeTableColumnReorderEvent} event - custom column reorder.
   * @group Emits
   */
  @Output() onColReorder: EventEmitter<TreeTableColumnReorderEvent> =
    new EventEmitter<TreeTableColumnReorderEvent>();
  /**
   * Callback to invoke when a node is selected.
   * @param {TreeTableNode} object - node instance.
   * @group Emits
   */
  @Output() onNodeSelect: EventEmitter<TreeTableNode> =
    new EventEmitter<TreeTableNode>();
  /**
   ...
   * @group Emits
   */
  @Output() onNodeUnselect: EventEmitter<TreeTableNodeUnSelectEvent> =
    new EventEmitter<TreeTableNodeUnSelectEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onEditInit: EventEmitter<TreeTableEditEvent> =
    new EventEmitter<TreeTableEditEvent>();
  /**
   * Callback to invoke when cell edit is completed.
   * @param {TreeTableEditEvent} event - custom edit event.
   * @group Emits
   */
  @Output() onEditComplete: EventEmitter<TreeTableEditEvent> =
    new EventEmitter<TreeTableEditEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onEditCancel: EventEmitter<TreeTableEditEvent> =
    new EventEmitter<TreeTableEditEvent>();
  ...
  multisortField(
    ...
    if (
      ObjectUtils.isEmpty(this.multiSortMeta) ||
  ...
  onColumnResizeEnd(event: MouseEvent, column: any) {
    let delta =
      (<ElementRef>this.resizeHelperViewChild).nativeElement.offsetLeft -
    ...
    if (columnWidth + delta > parseInt(minWidth)) {
      if (this.columnResizeMode === 'fit') {
        ...
        if (this.scrollable) {
          ...
          (<ElementRef>this.containerViewChild).nativeElement.style.width =
            containerWidth + 'px';
        }
      }
    ...
    (this.resizeHelperViewChild as ElementRef).nativeElement.style.display =
      'none';
  ...
  findParentScrollableView(column: any) {
    if (column) {
      ...
      while (
        parent &&
        !DomHandler.hasClass(parent, 'p-treetable-scrollable-view')
      ) {
  ...
  handleRowClick(event: any) {
    ...
    if (this.selectionMode) {
      ...
      let dataKeyValue = this.dataKey
        ? String(
            ObjectUtils.resolveFieldData(
              (<TreeTableNode>rowNode.node).data,
              this.dataKey
            )
          )
        : null;
        ...
        if (selected && metaKey) {
          if (this.isSingleSelectionMode()) {
            ...
            this._selection = this.selection.filter(
              (val: TreeTableNode, i: number) => i != selectionIndex
            );
  ...
  isFilterMatched(node: TreeTableNode, filterOptions: TreeTableFilterOptions) {
    let { filterField, filterValue, filterConstraint, isStrictMode } =
      filterOptions;
    let matched = false;
    let dataFieldValue = ObjectUtils.resolveFieldData(node.data, filterField);
    if (
      filterConstraint(dataFieldValue, filterValue, <string>this.filterLocale)
    ) {
      matched = true;
    }
    ...
    <ng-container *ngIf="!tt.virtualScroll">
      ...
      >
        <ng-container
          *ngTemplateOutlet="
            buildInItems;
            context: { $implicit: serializedValue, options: {} }
          "
        ></ng-container>
      </div>
    </ng-container>
...
export class TTScrollableView implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    if (!this.frozen) {
      ...
      (
        this.scrollHeaderBoxViewChild as ElementRef
      ).nativeElement.style.paddingRight = scrollBarWidth + 'px';
  ...
  onHeaderScroll() {
    ...
    (this.scrollBodyViewChild as ElementRef).nativeElement.scrollLeft =
      scrollLeft;
  ...
  onBodyScroll(event: any) {
    ...
    if (
      ...
      (
        this.scrollHeaderBoxViewChild as ElementRef
      ).nativeElement.style.marginLeft = -1 * event.target.scrollLeft + 'px';
    }
    ...
    if (
      ...
      (
        this.scrollFooterBoxViewChild as ElementRef
      ).nativeElement.style.marginLeft = -1 * event.target.scrollLeft + 'px';
    }
  ...
  scrollTo(options: ScrollToOptions): void {
    if (this.scroller) {
      ...
      if (this.scrollBodyViewChild?.nativeElement.scrollTo) {
        ...
        (this.scrollBodyViewChild as ElementRef).nativeElement.scrollLeft =
          options.left;
        (this.scrollBodyViewChild as ElementRef).nativeElement.scrollTop =
          options.top;
      }
    }
  }
...
export class TTSortIcon implements OnInit, OnDestroy {
  ...
  updateSortState() {
    if (this.tt.sortMode === 'single') {
      this.sortOrder = this.tt.isSorted(<string>this.field)
        ? this.tt.sortOrder
        : 0;
...
export class TTHeaderCheckbox {
  ...
  constructor(
    ...
    this.selectionChangeSubscription =
      this.tt.tableService.selectionSource$.subscribe(() => {
        this.checked = this.updateCheckedState();
      });
  }
    ...
    this.zone.runOutsideAngular(() => {
      setTimeout(() => {
        let row = DomHandler.findSingle(
          ...
          '.p-treetable-tbody'
        ).children[<number>this.tt.toggleRowIndex];
        if (row) {
          row.focus();
        }
      }, 25);
