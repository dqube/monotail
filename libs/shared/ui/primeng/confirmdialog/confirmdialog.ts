  ...
  template: `
    ...
    >
      ...
      >
        ...
        <div class="p-dialog-header" *ngIf="!headerTemplate">
          <span class="p-dialog-title" *ngIf="option('header')">{{
            option('header')
          }}</span>
        ...
        <div class="p-dialog-footer" *ngIf="!footer && !footerTemplate">
          ...
          >
            <ng-container *ngIf="!rejectIconTemplate">
              <i
                *ngIf="option('rejectIcon')"
                [class]="option('rejectIcon')"
              ></i>
              <TimesIcon
                *ngIf="!option('rejectIcon')"
          ...
          >
            <ng-container *ngIf="!acceptIconTemplate">
              <i
                *ngIf="option('acceptIcon')"
                [class]="option('acceptIcon')"
              ></i>
              <CheckIcon
                *ngIf="!option('acceptIcon')"
                [styleClass]="'p-button-icon-left'"
...
export class ConfirmDialog implements AfterContentInit, OnInit, OnDestroy {
          ...
          this.confirmationOptions = {
            ...
            closeOnEscape:
              ...
              this.confirmation.closeOnEscape === true
                ...
                : this.closeOnEscape,
            dismissableMask:
              this.confirmation.dismissableMask === false ||
              this.confirmation.dismissableMask === true
  ...
  moveOnTop() {
    if (this.autoZIndex) {
      ...
      (<HTMLElement>this.wrapper).style.zIndex = String(
        parseInt((<HTMLDivElement>this.container).style.zIndex, 10) - 1
      );
    }
  }

  getMaskClass() {
    let maskClass: { [key: string]: boolean } = {
          ...
          if (
            ...
            if (
              parseInt((this.container as HTMLDivElement).style.zIndex) ===
                ZIndexUtils.get(this.container) &&
              this.visible
            ) {
              this.close(event);
            }
  ...
  get rejectButtonLabel(): string {
    return (
      this.option('rejectLabel') ||
      this.config.getTranslation(TranslationKeys.REJECT)
    );
  }
}
