    ...
    >
        ...
        >
            ...
            <ng-container *ngIf="display === 'chip'">
              ...
              >
                ...
                <ng-container *ngIf="!disabled">
                  ...
                  >
                    <ng-container
                      *ngTemplateOutlet="removeTokenIconTemplate"
                    ></ng-container>
                  </span>
                </ng-container>
              </div>
          ...
          <ng-container
            *ngTemplateOutlet="
              ...
              context: { $implicit: value, removeChip: removeChip.bind(this) }
            "
          ></ng-container>
        </div>
        <ng-container *ngIf="value != null && filled && !disabled && showClear">
          <TimesIcon
            *ngIf="!clearIconTemplate"
            [styleClass]="'p-multiselect-clear-icon'"
            (click)="clear($event)"
          />
          <span
            *ngIf="clearIconTemplate"
      ...
      <div [ngClass]="{ 'p-multiselect-trigger': true }">
        <ng-container *ngIf="!dropdownIconTemplate">
          ...
          <ChevronDownIcon
            *ngIf="!dropdownIcon"
            [styleClass]="'p-multiselect-trigger-icon'"
          />
        </ng-container>
            ...
            <div class="p-multiselect-header" *ngIf="showHeader">
              ...
              <ng-template #builtInFilterElement>
                ...
                <div class="p-multiselect-filter-container" *ngIf="filter">
                  ...
                  />
                  <SearchIcon
                    [styleClass]="'p-multiselect-filter-icon'"
                ...
                <button
                  class="p-multiselect-close p-link p-button-icon-only"
                  type="button"
                  (click)="close($event)"
                  pRipple
                >
                  <TimesIcon
                    [styleClass]="'p-multiselect-close-icon'"
                    *ngIf="!closeIconTemplate"
                  />
                  <span
                    *ngIf="closeIconTemplate"
                    class="p-multiselect-close-icon"
                  >
                    <ng-template
    ...
    ControlValueAccessor
{
  ...
  isOptionDisabled(option: any) {
    let disabled = this.optionDisabled
      ...
      : false;
    return (
      disabled || (this.maxSelectionLimitReached && !this.isSelected(option))
    );
  }
  ...
  onOptionClick(event: { originalEvent: Event; option: any }) {
    ...
    if (selectionIndex != -1) {
      this.value = (this.value as any[]).filter(
  ...
  onMouseclick(event: MouseEvent, input: HTMLInputElement) {
    if (
      this.disabled ||
      this.readonly ||
      (<Node>event.target).isSameNode(this.accessibleViewChild?.nativeElement)
    ) {
      return;
    }
  ...
  removeChip(chip: MultiSelectItem, event: MouseEvent) {
    this.value = (<any[]>this.value).filter(
      (val) => !ObjectUtils.equals(val, chip, this.dataKey)
    );
    this.onModelChange(this.value);
    this.checkSelectionLimit();
    this.onChange.emit({
    ...
    switch ((<KeyboardEvent>event.originalEvent).which) {
      ...
      case 40:
        var nextItem = this.findNextItem(
          (event.originalEvent.target as any).parentElement
        );
        if (nextItem) {
          nextItem.focus();
        }
      ...
      if (
        ...
        if (pattern.test(this.selectedItemsLabel)) {
          this.valuesAsString = this.selectedItemsLabel.replace(
            (this.selectedItemsLabel as any).match(pattern)[0],
            this.value.length + ''
          );
        } else {
  ...
  findLabelByValue(val: any): string {
    if (this.group) {
      ...
      for (let i = 0; i < (this.options as any[]).length; i++) {
        let subOptions = this.getOptionGroupChildren(
          (this.options as any[])[i]
        );
        if (subOptions) {
