...
export class UITreeNode implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.parentNode) {
      this.tree.syncNodeOption(
        <TreeNode>this.node,
        <TreeNode[]>this.tree.value,
        'parent',
        this.tree.getNodeWithKey(
          <string>this.parentNode.key,
          <TreeNode[]>this.tree.value
        )
      );
    }
  ...
  onDropPoint(event: DragEvent, position: number) {
    ...
    let isValidDropPointIndex =
      this.tree.dragNodeTree === this.tree
        ? position === 1 || dragNodeIndex !== <number>this.index - 1
        : true;
    ...
    if (
      ...
      let dropParams = {
        ...this.createDropPointEventMetadata(<number>position),
      };
  ...
  processPointDrop(event: any) {
    let newNodeList = event.dropNode.parent
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    if (
      nodeElement?.nodeName !== 'P-TREENODE' ||
    ...
    switch (event.which) {
      ...
      case 39:
        if (
          !this.node?.expanded &&
          !this.tree.isNodeLeaf(<TreeNode>this.node)
        ) {
          this.expand(event);
        }
        event.preventDefault();
  ...
  focusNode(element: any) {
    if (this.tree.droppableNodes)
      (element.children[1].children[0] as HTMLElement).focus();
  ...
  implements OnInit, AfterContentInit, OnChanges, OnDestroy, BlockableUI
{
   ...
   * @group Emits
   */
  @Output() selectionChange: EventEmitter<TreeNode[] | TreeNode | null> =
    new EventEmitter<TreeNode[] | TreeNode | null>();
  /**
   ...
   * @group Emits
   */
  @Output() onNodeExpand: EventEmitter<TreeNodeExpandEvent> =
    new EventEmitter<TreeNodeExpandEvent>();
  /**
   * Callback to invoke when a node is collapsed.
   * @param {TreeNodeCollapseEvent} event - node collapse event.
   * @group Emits
   */
  @Output() onNodeCollapse: EventEmitter<TreeNodeCollapseEvent> =
    new EventEmitter<TreeNodeCollapseEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onNodeDrop: EventEmitter<TreeNodeDropEvent> =
   ...
   * @group Emits
   */
  @Output() onLazyLoad: EventEmitter<TreeLazyLoadEvent> =
    new EventEmitter<TreeLazyLoadEvent>();
  /**
   * Callback to invoke in virtual scroll mode when scroll position changes.
   * @param {TreeScrollEvent} event - custom scroll event.
   * @group Emits
   */
  @Output() onScroll: EventEmitter<TreeScrollEvent> =
    new EventEmitter<TreeScrollEvent>();
  /**
  ...
  onNodeClick(event: Event, node: TreeNode) {
    ...
    if (
      ...
      if (this.isCheckboxSelectionMode()) {
        ...
        if (metaSelection) {
          let metaKey =
            (<KeyboardEvent>event).metaKey || (<KeyboardEvent>event).ctrlKey;
  ...
  syncNodeOption(
    ...
    const _node = this.hasFilteredNodes()
      ? this.getNodeWithKey(<string>node.key, parentNodes)
      : null;
    if (_node) {
  ...
  onDragOver(event: DragEvent) {
    if (
      this.droppableNodes &&
      (!this.value || (<any>this.value).length === 0)
    ) {
  ...
  onDragEnter() {
    if (
      this.droppableNodes &&
      this.allowDrop(<TreeNode>this.dragNode, null, this.dragNodeScope)
    ) {
      this.dragHover = true;
    }
  }
