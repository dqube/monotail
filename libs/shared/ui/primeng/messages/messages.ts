      ...
      <ng-container *ngIf="!contentTemplate; else staticMessage">
        ...
        >
          <div class="p-message-wrapper">
            <span *ngIf="msg.icon" [class]="'p-message-icon pi ' + msg.icon">
            </span>
            ...
            <ng-template #escapeOut>
              <span *ngIf="msg.summary" class="p-message-summary">{{
                msg.summary
              }}</span>
              <span *ngIf="msg.detail" class="p-message-detail">{{
                msg.detail
              }}</span>
            </ng-template>
            <button
...
export class Messages implements AfterContentInit, OnDestroy {
  ...
  get icon(): string | null {
    const severity =
      this.severity || (this.hasMessages() ? this.messages![0].severity : null);
  ...
  private startMessageLifes(messages: Message[]): void {
    messages?.forEach(
      (message) => message.life && this.startMessageLife(message)
    );
  }
  ...
  private startMessageLife(message: Message): void {
    const timerSubsctiption = timer(message.life!).subscribe(() => {
      ...
      this.timerSubscriptions = this.timerSubscriptions?.filter(
        (timerEl) => timerEl !== timerSubsctiption
      );
      this.valueChange.emit(this.messages);
      this.cd.markForCheck();
    });
