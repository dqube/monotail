        ...
        >
          ...
          >
            ...
            <ng-container *ngIf="child.items">
              ...
              >
                <ng-template
                  *ngTemplateOutlet="contextMenu.submenuIconTemplate"
                ></ng-template>
              </span>
            </ng-container>
          </a>
...
export class ContextMenuSub {
  ...
  ngOnInit() {
    this.activeItemKeyChangeSubscription =
      this.contextMenu.contextMenuService.activeItemKeyChange$
        .pipe(takeUntil(this.contextMenu.ngDestroy$))
        .subscribe((activeItemKey) => {
          this.activeItemKey = activeItemKey;

          if (
            this.isActive(this.parentItemKey) &&
            DomHandler.hasClass(
              this.sublistViewChild?.nativeElement,
              'p-submenu-list-active'
            )
          ) {
            this.contextMenu.positionSubmenu(
              this.sublistViewChild?.nativeElement
            );
          }
  ...
  onItemMouseEnter(event: Event, item: MenuItem, key: string) {
    ...
    if (item.items) {
      let childSublist = DomHandler.findSingle(
        event.currentTarget,
        '.p-submenu-list'
      );
  ...
  onItemMouseLeave(event: MouseEvent, item: MenuItem) {
    ...
    if (this.contextMenu.el.nativeElement.contains(<Node>event.target)) {
      if (item.items) {
        this.contextMenu.removeActiveFromSubLists(
  ...
  onItemClick(
    ...
    if (item.items) {
      ...
      if (childSublist) {
        if (
          this.isActive(key) &&
          DomHandler.hasClass(childSublist, 'p-submenu-list-active')
        ) {
  ...
  isActive(key: string) {
    return (
      this.activeItemKey &&
      (this.activeItemKey.startsWith(key + '_') || this.activeItemKey === key)
    );
  }
}
...
@Component({
  ...
  template: `
    <div
      #container
      [ngClass]="'p-contextmenu p-component'"
      [class]="styleClass"
      [ngStyle]="style"
    >
...
export class ContextMenu implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    if (this.global) {
      const documentTarget: any = this.el
        ...
        : 'document';
      this.triggerEventListener = this.renderer.listen(
        documentTarget,
        this.triggerEvent,
        (event) => {
          if (
  ...
  toggle(event?: MouseEvent) {
    if ((<ElementRef>this.containerViewChild).nativeElement.offsetParent)
      this.hide();
  ...
  position(event?: MouseEvent) {
    if (event) {
      ...
      //flip
      if (
        left + width - (<any>this.document).scrollingElement.scrollLeft >
        viewport.width
      ) {
        left -= width;
      }
      ...
      //flip
      if (
        top + height - (<any>this.document).scrollingElement.scrollTop >
        viewport.height
      ) {
        top -= height;
      }
      ...
      (<ElementRef>this.containerViewChild).nativeElement.style.left =
        left + 'px';
      (<ElementRef>this.containerViewChild).nativeElement.style.top =
        top + 'px';
    }
  }
  ...
  positionSubmenu(sublist: HTMLElement) {
    ...
    let viewport = DomHandler.getViewport();
    let sublistWidth = sublist.offsetParent
      ? sublist.offsetWidth
      : DomHandler.getHiddenElementOuterWidth(sublist);
    let sublistHeight = sublist.offsetHeight
      ? sublist.offsetHeight
      : DomHandler.getHiddenElementOuterHeight(sublist);
    let itemOuterWidth = DomHandler.getOuterWidth(parentMenuItem?.children[0]);
    let itemOuterHeight = DomHandler.getOuterHeight(
      parentMenuItem?.children[0]
    );
    ...
    if (
      parseInt(containerOffset.top) + itemOuterHeight + sublistHeight >
      viewport.height - DomHandler.calculateScrollbarHeight()
    ) {
    ...
    if (
      parseInt(containerOffset.left) + itemOuterWidth + sublistWidth >
  ...
  isItemMatched(menuitem: Element) {
    return (
      DomHandler.hasClass(menuitem, 'p-menuitem') &&
      !DomHandler.hasClass(menuitem.children[0], 'p-disabled')
    );
  }
  ...
  findNextItem(menuitem: Element, isRepeated?: boolean): any {
    ...
    if (nextMenuitem) {
      return this.isItemMatched(nextMenuitem)
        ? nextMenuitem
        : this.findNextItem(nextMenuitem, isRepeated);
    } else {
      let firstItem = menuitem.parentElement?.children[0];

      return this.isItemMatched(firstItem!)
        ? firstItem
        : !isRepeated
        ? this.findNextItem(firstItem as Element, true)
        : null;
    }
  }
  ...
  findPrevItem(menuitem: Element, isRepeated?: boolean): any {
    ...
    if (prevMenuitem) {
      return this.isItemMatched(prevMenuitem)
        ? prevMenuitem
        : this.findPrevItem(prevMenuitem, isRepeated);
    } else {
      let lastItem =
        menuitem.parentElement?.children[
          menuitem.parentElement.children.length - 1
        ];

      return this.isItemMatched(<Element>lastItem)
        ? lastItem
        : !isRepeated
        ? this.findPrevItem(<Element>lastItem, true)
        : null;
    }
  }
  ...
  getActiveItem() {
    ...
    return activeItemKey == null
      ...
      : DomHandler.findSingle(
          this.containerViewChild?.nativeElement,
          '.p-menuitem[data-ik="' + activeItemKey + '"]'
        );
  }
  ...
  bindGlobalListeners() {
    if (!this.documentClickListener) {
      ...
      this.documentTriggerListener = this.renderer.listen(
        documentTarget,
        this.triggerEvent,
        (event) => {
          if (
          ...
          switch (event.key) {
            case 'ArrowDown':
              if (activeItem) {
                ...
                let firstItem = DomHandler.findSingle(
                  this.containerViewChild?.nativeElement,
                  '.p-menuitem-link'
                ).parentElement;
                activeItem = this.isItemMatched(firstItem)
                  ? firstItem
                  : this.findNextItem(firstItem);
              }

              if (activeItem) {
                this.contextMenuService.changeKey(
                  activeItem.getAttribute('data-ik')
                );
              }
            ...
            case 'ArrowUp':
              if (activeItem) {
                ...
                let sublist = DomHandler.findSingle(
                  this.containerViewChild?.nativeElement,
                  'ul'
                );
                let lastItem = sublist.children[sublist.children.length - 1];
                activeItem = this.isItemMatched(lastItem)
                  ? lastItem
                  : this.findPrevItem(lastItem);
              }
            ...
            case 'ArrowRight':
              if (activeItem) {
                ...
                if (sublist) {
                  ...
                  activeItem = DomHandler.findSingle(
                    sublist,
            ...
            case 'ArrowLeft':
              if (activeItem) {
                ...
                if (
                  sublist &&
                  DomHandler.hasClass(sublist, 'p-submenu-list-active')
                ) {
                  DomHandler.removeClass(sublist, 'p-submenu-list-active');
  ...
  findModelItemFromKey(key: string) {
    ...
    return indexes.reduce((item: any, currentIndex: any) => {
      return item
        ...
        : (this.model as MenuItem[])[currentIndex];
    }, null);
  }
  ...
  isOutsideClicked(event: Event) {
    return !(
      this.containerViewChild?.nativeElement.isSameNode(event.target) ||
