...
export class Splitter {
  ...
  set panelSizes(val: number[]) {
    ...
    if (this.el && this.el.nativeElement && this.panels.length > 0) {
      let children = [...this.el.nativeElement.children[0].children].filter(
        (child) => DomHandler.hasClass(child, 'p-splitter-panel')
      );
      let _panelSizes = [];

      this.panels.map((panel, i) => {
        let panelInitialSize =
          this.panelSizes.length - 1 >= i ? this.panelSizes[i] : null;
   ...
   * @group Emits
   */
  @Output() onResizeEnd: EventEmitter<SplitterResizeEndEvent> =
    new EventEmitter<SplitterResizeEndEvent>();
  /**
   * Callback to invoke when resize starts.
   * @param {SplitterResizeStartEvent} event - Custom panel resize start event
   * @group Emits
   */
  @Output() onResizeStart: EventEmitter<SplitterResizeStartEvent> =
    new EventEmitter<SplitterResizeStartEvent>();
      ...
      if (!initialized) {
        ...
        this.panels.map((panel, i) => {
          ...
          children[i].style.flexBasis =
            ...
            panelSize +
            '% - ' +
            (this.panels.length - 1) * (this.gutterSize as number) +
            'px)';
        });
  ...
  resizeStart(event: TouchEvent | MouseEvent, index: number) {
    ...
    this.size = this.horizontal()
      ? DomHandler.getWidth(
          (this.containerViewChild as ElementRef).nativeElement
        )
    ...
    this.prevPanelElement = this.gutterElement
      .previousElementSibling as HTMLElement;
    this.nextPanelElement = this.gutterElement
    ...
    this.onResizeStart.emit({
      originalEvent: event,
      sizes: this._panelSizes as number[],
    });
  }
  ...
  onResize(event: MouseEvent) {
    ...
    if (this.horizontal())
      newPos =
        (event.pageX * 100) / (this.size as number) -
        ((this.startPos as number) * 100) / (this.size as number);
    else
      newPos =
        (event.pageY * 100) / (this.size as number) -
        ((this.startPos as number) * 100) / (this.size as number);
  ...
  validateResize(newPrevPanelSize: number, newNextPanelSize: number) {
    if (
      this.minSizes.length >= 1 &&
      this.minSizes[0] &&
      this.minSizes[0] > newPrevPanelSize
    ) {
      return false;
    }
    ...
    if (
      this.minSizes.length > 1 &&
      this.minSizes[1] &&
      this.minSizes[1] > newNextPanelSize
    ) {
      return false;
    }
  ...
  saveState() {
    this.getStorage().setItem(
      this.stateKey as string,
      JSON.stringify(this._panelSizes)
    );
  }
  ...
  restoreState() {
    ...
    if (stateString) {
      ...
      let children = [
        ...(this.containerViewChild as ElementRef).nativeElement.children,
      ].filter((child) => DomHandler.hasClass(child, 'p-splitter-panel'));
