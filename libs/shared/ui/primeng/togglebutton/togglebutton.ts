  ...
  template: `
    <div
      [ngClass]="{
        ...
        'p-button-icon-only': onIcon && offIcon && !hasOnLabel && !hasOffLabel,
        'p-highlight': checked,
        'p-disabled': disabled
      }"
      [ngStyle]="style"
    ...
    >
      ...
      ></span>
      <span class="p-button-label" *ngIf="onLabel || offLabel">{{
...
export class ToggleButton implements ControlValueAccessor {
   ...
   * @group Emits
   */
  @Output() onChange: EventEmitter<ToggleButtonChangeEvent> =
    new EventEmitter<ToggleButtonChangeEvent>();
