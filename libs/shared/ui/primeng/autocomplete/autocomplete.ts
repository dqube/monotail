                    ...
                    *ngTemplateOutlet="
                      ...
                      context: {
                        ...
                        index: scrollerOptions.getOptions
                          ? scrollerOptions.getOptions(idx)
                          : idx
                      }
                    "
                  ></ng-container>
    ...
    ControlValueAccessor
{
        ...
        setTimeout(() => {
          if (this.overlayViewChild && this.itemsWrapper) {
            let listItem = DomHandler.findSingle(
              (this.overlayViewChild.overlayViewChild as ElementRef)
                .nativeElement,
              'li.p-highlight'
            );
  ...
  removeItem(item: any) {
    ...
    this.value = (this.value as object[]).filter(
      (val: any, i: number) => i != itemIndex
    );
    this.onModelChange(this.value);
    this.updateFilledState();
  ...
  onKeydown(event: Event) {
    if (this.overlayVisible) {
      ...
      if ((<KeyboardEvent>event).which === 40 && this.suggestions) {
        ...
        (<KeyboardEvent>event).key === 'z' &&
        !this.multiple
      ) {
        (this.inputEL as ElementRef).nativeElement.value =
          this.resolveFieldData(null);
        this.value = '';
        this.onModelChange(this.value);
      } else if (
        (<KeyboardEvent>event).ctrlKey &&
