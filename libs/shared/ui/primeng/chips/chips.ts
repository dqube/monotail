    ...
    >
      ...
      >
        ...
        <li
          ...
          [ngClass]="{ 'p-chips-clearable': showClear && !disabled }"
        >
          <input
            #inputtext
            type="text"
        ...
        <li *ngIf="value != null && filled && !disabled && showClear">
          <TimesIcon
            *ngIf="!clearIconTemplate"
            [styleClass]="'p-chips-clear-icon'"
            (click)="clear()"
          />
          <span
            *ngIf="clearIconTemplate"
            class="p-chips-clear-icon"
            (click)="clear()"
          >
...
export class Chips implements AfterContentInit, ControlValueAccessor {
   ...
   * @group Emits
   */
  @Output() onAdd: EventEmitter<ChipsAddEvent> =
    new EventEmitter<ChipsAddEvent>();
  /**
   * Callback to invoke on chip remove.
   * @param {ChipsRemoveEvent} event - Custom event.
   * @group Emits
   */
  @Output() onRemove: EventEmitter<ChipsRemoveEvent> =
    new EventEmitter<ChipsRemoveEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onChipClick: EventEmitter<ChipsClickEvent> =
    new EventEmitter<ChipsClickEvent>();
  /**
  ...
  onPaste(event: any) {
    if (!this.disabled) {
      if (this.separator) {
        let pastedData = (
          event.clipboardData ||
          (this.document.defaultView as any)['clipboardData']
        ).getData('Text');
  ...
  onKeydown(event: KeyboardEvent): void {
    switch (event.which) {
      ...
      case 8:
        if (
          this.inputViewChild.nativeElement.value.length === 0 &&
          this.value &&
          this.value.length > 0
        ) {
          this.value = [...this.value];
