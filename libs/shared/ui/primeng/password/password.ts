...
export class PasswordDirective implements OnDestroy, DoCheck {
  ...
  onKeyup(e: Event) {
    if (this.feedback) {
      ...
      if (
        !this.panel ||
        !DomHandler.hasClass(this.panel, 'p-connected-overlay-visible')
      ) {
        this.showOverlay();
      }
...
@Component({
  ...
  template: `
    <div
      [ngClass]="toggleMask | mapper : containerClass"
      [ngStyle]="style"
      [class]="styleClass"
    >
      <input
      ...
      <ng-container *ngIf="showClear && value != null">
        <TimesIcon
          *ngIf="!clearIconTemplate"
          [styleClass]="'p-password-clear-icon'"
          (click)="clear()"
        />
...
export class Password implements AfterContentInit, OnInit {
   ...
   * @group Emits
   */
  @Output() onClear: EventEmitter<undefined | null> = new EventEmitter<
    undefined | null
  >();
    ...
    if (this.appendTo) {
      if (this.appendTo === 'body')
        ...
        (this.document as any)
          ...
          .appendChild(this.overlay as HTMLElement);
    }
  }

  alignOverlay() {
    if (this.appendTo) {
      (this.overlay as HTMLElement).style.minWidth =
        DomHandler.getOuterWidth(this.input.nativeElement) + 'px';
  ...
  promptText() {
    return (
      this.promptLabel || this.getTranslation(TranslationKeys.PASSWORD_PROMPT)
    );
  }
