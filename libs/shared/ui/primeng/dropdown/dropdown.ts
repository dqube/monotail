  ...
  template: `
    ...
    >
      ...
      <ng-container *ngIf="isVisibleClearIcon">
        <TimesIcon
          [styleClass]="'p-dropdown-clear-icon'"
          (click)="clear($event)"
      ...
      >
        <ng-container *ngIf="!dropdownIconTemplate">
          ...
          <ChevronDownIcon
            *ngIf="!dropdownIcon"
            [styleClass]="'p-dropdown-trigger-icon'"
          />
        </ng-container>
                ...
                <div class="p-dropdown-filter-container">
                  ...
                  <SearchIcon
                    *ngIf="!filterIconTemplate"
                    [styleClass]="'p-dropdown-filter-icon'"
                  />
                  <span
                    *ngIf="filterIconTemplate"
    ...
    ControlValueAccessor
{
  ...
  @Output() onChange: EventEmitter<DropdownChangeEvent> =
    new EventEmitter<DropdownChangeEvent>();
  /**
   * Callback to invoke when data is filtered.
   * @param {DropdownFilterEvent} event - custom filter event.
   * @group Emits
   */
  @Output() onFilter: EventEmitter<DropdownFilterEvent> =
    new EventEmitter<DropdownFilterEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onShow: EventEmitter<AnimationEvent> =
   ...
   * @group Emits
   */
  @Output() onHide: EventEmitter<AnimationEvent> =
    new EventEmitter<AnimationEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onLazyLoad: EventEmitter<DropdownLazyLoadEvent> =
    new EventEmitter<DropdownLazyLoadEvent>();
  ...
  isInputClick(event: MouseEvent): boolean {
    return (
      ...
      (event.target as HTMLInputElement).isSameNode(
  ...
  searchOption(index: number) {
    ...
    if (this.searchValue) {
      option = this.searchOptionInRange(
        index,
        (this.optionsToDisplay as any[]).length
      );
        ...
        if (!option) {
          for (let i = 0; i <= index.groupIndex; i++) {
            for (
              ...
              let opt = this.getOptionGroupChildren(this.optionsToDisplay[i])[
                j
              ];
              if (
                this.getOptionLabel(opt)
    ...
    if (opts) {
      for (let i = 0; i < opts.length; i++) {
        if (
          ...
          ObjectUtils.equals(val, this.getOptionValue(opts[i]), this.dataKey)
        ) {
          index = i;
          break;
        }
