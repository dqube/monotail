        ...
        >
          <a
            *ngIf="isClickableRouterLink(item); else elseBlock"
            ...
            [attr.id]="item.id"
            [attr.tabindex]="
              item.disabled || readonly || !visible
                ? null
                : item.tabindex
                ? item.tabindex
                : '0'
...
export class SpeedDial implements AfterViewInit, AfterContentInit, OnDestroy {
   ...
   * @group Emits
   */
  @Output() onVisibleChange: EventEmitter<boolean> =
    ...
    if (isPlatformBrowser(this.platformId)) {
      if (this.type !== 'linear') {
        ...
        if (button && firstItem) {
          ...
          this.list?.nativeElement.style.setProperty(
            '--item-diff-x',
            `${wDiff / 2}px`
          );
          this.list?.nativeElement.style.setProperty(
  ...
  get buttonIconClass() {
    return (!this.visible && this.showIcon) || !this.hideIcon
      ? this.showIcon
      : this.hideIcon;
  }
