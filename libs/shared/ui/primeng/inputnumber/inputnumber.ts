...
@Component({
  ...
  template: `
    ...
    >
      ...
      >
        ...
        >
          <span
            *ngIf="incrementButtonIcon"
            [ngClass]="incrementButtonIcon"
          ></span>
        ...
        >
          <span
            *ngIf="decrementButtonIcon"
            [ngClass]="decrementButtonIcon"
          ></span>
  ...
  host: {
    ...
    '[class.p-inputnumber-clearable]':
      'showClear && buttonLayout != "vertical"',
  },
})
  ...
  implements OnInit, AfterContentInit, OnChanges, ControlValueAccessor
{
  ...
  onInputKeyDown(event: KeyboardEvent) {
    ...
    let selectionStart = (event.target as HTMLInputElement)
      .selectionStart as number;
    let selectionEnd = (event.target as HTMLInputElement)
    ...
    switch (event.which) {
      ...
      //enter
      case 13:
        newValueStr = this.validateValue(
          ...
          if (this.isNumeralChar(deleteChar)) {
            ...
            if (this._group.test(deleteChar)) {
              ...
              newValueStr =
                ...
                inputValue.slice(selectionStart);
              newValueStr =
                (this.parseValue(newValueStr) as number) > 0 ? newValueStr : '';
            } else {
              newValueStr =
                inputValue.slice(0, selectionStart - 1) +
                inputValue.slice(selectionStart);
  ...
  onPaste(event: ClipboardEvent) {
    if (!this.disabled && !this.readonly) {
      ...
      let data = (
        event.clipboardData ||
        (this.document as any).defaultView['clipboardData']
      ).getData('Text');
      if (data) {
  ...
  handleOnInput(event: Event, currentValue: string, newValue: any) {
    if (this.isValueChanged(currentValue, newValue)) {
      (this.input as ElementRef).nativeElement.value =
