...
@Component({
  ...
  template: `
    <ul
      #sublist
      [ngClass]="{
        'p-slidemenu-rootlist': root,
        'p-submenu-list': !root,
        'p-active-submenu': isActive
      }"
      ...
      <ng-template ngFor let-child [ngForOf]="root ? item : item?.items">
        ...
        <li
          *ngIf="!child.separator"
          ...
          [ngClass]="{
            'p-menuitem': true,
            'p-menuitem-active': listitem == activeItem,
        ...
        >
          ...
          >
            ...
            <ng-container *ngIf="child.items">
              <AngleRightIcon
                *ngIf="!slideMenu.submenuIconTemplate"
                [styleClass]="'p-submenu-icon'"
              />
              <ng-template
                *ngTemplateOutlet="slideMenu.submenuIconTemplate"
              ></ng-template>
            </ng-container>
          </a>
          <a
...
export class SlideMenuSub implements OnDestroy {
  ...
  constructor(
    @Inject(forwardRef(() => SlideMenu)) slideMenu: SlideMenu,
      ...
      case 'Enter':
        if (listItem && !DomHandler.hasClass(listItem, 'p-disabled')) {
          ...
          this.transitionEndListener = this.renderer.listen(
            ...
            'transitionend',
            this.focusNextList.bind(this, listItem)
          );
        }
  ...
  get isActive() {
    return (
      -this.slideMenu.left ==
      (this.index as number) * (this.menuWidth as number)
    );
  }
}
  ...
  template: `
    ...
    >
      <div
        ...
        [style.height]="left ? viewportHeight + 'px' : 'auto'"
      ...
      >
        ...
        <a
          #backward
          (keydown.enter)="onBackwardKeydown($event)"
          (keydown.space)="onBackwardKeydown($event)"
        ...
        >
          <CaretLeftIcon
            *ngIf="!backIconTemplate"
            [styleClass]="'p-slidemenu-backward-icon'"
          />
  ...
  implements AfterViewChecked, AfterContentInit, OnDestroy
{
  ...
  updateViewPort() {
    this.renderer.setStyle(
      this.slideMenuContentViewChild?.nativeElement,
      'height',
      this.viewportHeight -
        DomHandler.getHiddenElementOuterHeight(
          this.backwardViewChild?.nativeElement
        ) +
        'px'
    );
  }
