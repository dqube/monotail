...
describe('SlideMenu', () => {
  ...
  beforeEach(() => {
    ...
    slidemenuSub = fixture.debugElement.children[0].query(
      By.css('p-slideMenuSub')
    ).componentInstance;
  });
  ...
  it('should change backlabel', () => {
    ...
    slidemenu.cd.detectChanges();
    const backwardSpanEl = fixture.debugElement.query(
      By.css('.p-slidemenu-backward')
    ).children[1];
    expect(backwardSpanEl.nativeElement.textContent).toEqual(
      'ALWAYS BET ON PRIME'
    );
  });
  ...
  it('should change menuWidth effectDuration and easing', () => {
    ...
    slidemenu.cd.detectChanges();
    const slideMenuSubEl = fixture.debugElement
      .query(By.css('p-slideMenuSub'))
      .query(By.css('ul'));
    expect(slidemenuSub.easing).toEqual(slidemenu.easing);
    expect(slidemenuSub.effectDuration).toEqual(slidemenu.effectDuration);
    expect(slidemenuSub.easing).toEqual(slidemenu.easing);
    expect(slideMenuSubEl.nativeElement.style.transitionTimingFunction).toEqual(
      slidemenu.easing
    );
    expect(slideMenuSubEl.nativeElement.style.transitionDuration).toEqual(
      slidemenu.effectDuration + 'ms'
    );
    expect(slideMenuSubEl.nativeElement.style.width).toEqual(
      slidemenu.menuWidth + 'px'
    );
  });
  ...
  it('should show items', () => {
    ...
    const firstSlidemenuSubEl = fixture.debugElement
      .query(By.css('p-slideMenuSub'))
      .query(By.css('ul'));
    const itemsEl = fixture.debugElement
      .query(By.css('p-slideMenuSub'))
      .queryAll(By.css('li'));
    ...
    for (let item of slidemenu.model) {
      expect(item.label).toEqual(
        itemsEl[i].query(By.css('.p-menuitem-text')).nativeElement.textContent
      );
      i++;
      if (item.items) {
        for (let child of item.items as MenuItem[]) {
          if (child.label)
            expect(child.label).toEqual(
              itemsEl[i].query(By.css('.p-menuitem-text')).nativeElement
                .textContent
            );
          i++;
        }
  ...
  it('should call itemClick when click and change menu from root to submenu', () => {
    ...
    expect(
      activeItem.query(By.css('.p-menuitem-text')).nativeElement.textContent
    ).toEqual('File');
    expect(itemClickSpy).toHaveBeenCalled();
    expect(activeItem.query(By.css('ul')).nativeElement.className).toContain(
      'p-active-submenu'
    );
  ...
  it('should call goBack when click and change menu from subMenu to root', () => {
    ...
    expect(subMenu.nativeElement.className).not.toContain(
      'p-submenu-list p-active-submenu'
    );
    expect(rootMenu.nativeElement.className).toContain(
