...
export class Galleria implements OnChanges, OnDestroy {
   ...
   * @group Props
   */
  @Input() thumbnailsPosition: 'bottom' | 'top' | 'left' | 'right' | undefined =
    'bottom';
  /**
   ...
   * @group Props
   */
  @Input() indicatorsPosition: 'bottom' | 'top' | 'left' | 'right' | undefined =
    'bottom';
  /**
  ...
  ngOnChanges(simpleChanges: SimpleChanges) {
    if (
      simpleChanges.value &&
      simpleChanges.value.currentValue?.length < this.numVisible
    ) {
...
@Component({
  ...
  template: `
      ...
      *ngIf="value && value.length > 0"
      [ngClass]="{
        ...
        'p-galleria-item-nav-onhover':
          this.galleria.showItemNavigatorsOnHover && !this.galleria.fullScreen
      }"
    ...
    >
      ...
      >
        <TimesIcon
          *ngIf="!galleria.closeIconTemplate"
          [styleClass]="'p-galleria-close-icon'"
        />
        <ng-template
          *ngTemplateOutlet="galleria.closeIconTemplate"
        ></ng-template>
      </button>
      <div
        *ngIf="galleria.templates && galleria.headerFacet"
      ...
      <div
        *ngIf="galleria.templates && galleria.footerFacet"
...
export class GalleriaContent implements DoCheck {
  ...
  ngDoCheck(): void {
    const changes = this.differ.diff(
      this.galleria as unknown as Record<string, unknown>
    );
  ...
  galleriaClass() {
    const thumbnailsPosClass =
      this.galleria.showThumbnails &&
      this.getPositionClass(
        'p-galleria-thumbnails',
        this.galleria.thumbnailsPosition as string
      );
    const indicatorPosClass =
      this.galleria.showIndicators &&
      this.getPositionClass(
        'p-galleria-indicators',
        this.galleria.indicatorsPosition as string
      );
  ...
  startSlideShow() {
    this.interval = setInterval(() => {
      let activeIndex =
        this.galleria.circular && this.value.length - 1 === this.activeIndex
          ? 0
          : this.activeIndex + 1;
  ...
  isVertical() {
    return (
      this.galleria.thumbnailsPosition === 'left' ||
      this.galleria.thumbnailsPosition === 'right'
    );
  }
...
@Component({
  ...
  template: `
    <ng-container *ngIf="contentTemplate">
      <ng-container
        *ngTemplateOutlet="contentTemplate; context: context"
      ></ng-container>
    </ng-container>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  ...
  template: `
    <div class="p-galleria-item-wrapper">
      <div class="p-galleria-item-container">
        ...
        >
          <ChevronLeftIcon
            *ngIf="!galleria.itemPreviousIconTemplate"
            [styleClass]="'p-galleria-item-prev-icon'"
          />
          <ng-template
            *ngTemplateOutlet="galleria.itemPreviousIconTemplate"
          ></ng-template>
        </button>
        <p-galleriaItemSlot
          type="item"
        ...
        >
          <ChevronRightIcon
            *ngIf="!galleria.itemNextIconTemplate"
            [styleClass]="'p-galleria-item-next-icon'"
          />
          <ng-template
            *ngTemplateOutlet="galleria.itemNextIconTemplate"
          ></ng-template>
        </button>
        ...
        <li
          *ngFor="let item of value; let index = index"
          ...
          [ngClass]="{
            ...
            'p-highlight': isIndicatorItemActive(index)
          }"
        >
          <button
            type="button"
...
export class GalleriaItem implements OnChanges {
  ...
  next() {
    ...
    let activeIndex =
      this.circular && (<any[]>this.value).length - 1 === this.activeIndex
        ? 0
        : nextItemIndex;
  ...
  prev() {
    ...
    let activeIndex =
      this.circular && this.activeIndex === 0
        ? (<any[]>this.value).length - 1
        : prevItemIndex;
    this.onActiveIndexChange.emit(activeIndex);
  }
  ...
  isNavForwardDisabled() {
    return (
      !this.circular && this.activeIndex === (<any[]>this.value).length - 1
    );
  }
      ...
      <div class="p-galleria-thumbnail-container">
        ...
        >
          <ng-container *ngIf="!galleria.previousThumbnailIconTemplate">
            <ChevronLeftIcon
              *ngIf="!isVertical"
              [styleClass]="'p-galleria-thumbnail-prev-icon'"
            />
            <ChevronUpIcon
              *ngIf="isVertical"
              [styleClass]="'p-galleria-thumbnail-prev-icon'"
          ...
          <ng-template
            *ngTemplateOutlet="galleria.previousThumbnailIconTemplate"
          ></ng-template>
        </button>
        <div
          class="p-galleria-thumbnail-items-container"
          [ngStyle]="{ height: isVertical ? contentHeight : '' }"
        >
          <div
            #itemsContainer
        ...
        >
          <ng-container *ngIf="!galleria.nextThumbnailIconTemplate">
            <ChevronRightIcon
              *ngIf="!isVertical"
              [ngClass]="'p-galleria-thumbnail-next-icon'"
            />
            <ChevronDownIcon
              *ngIf="isVertical"
              [ngClass]="'p-galleria-thumbnail-next-icon'"
          ...
          <ng-template
            *ngTemplateOutlet="galleria.nextThumbnailIconTemplate"
          ></ng-template>
        </button>
      </div>
    </div>
  ...
  implements OnInit, AfterContentChecked, AfterViewInit, OnDestroy
{
  ...
  ngAfterContentChecked() {
    ...
    if (
      ...
      if (this._activeIndex <= this.getMedianItemIndex()) {
        ...
        (<any[]>this.value).length -
          this.d_numVisible +
          this.getMedianItemIndex() <
        this._activeIndex
      ) {
        totalShiftedItems = this.d_numVisible - (<any[]>this.value).length;
      } else if (
        (<any[]>this.value).length - this.d_numVisible < this._activeIndex &&
        this.d_numVisible % 2 === 0
      ) {
        totalShiftedItems =
          this._activeIndex * -1 + this.getMedianItemIndex() + 1;
      } else {
      ...
      if (this.itemsContainer && this.itemsContainer.nativeElement) {
        this.itemsContainer.nativeElement.style.transform = this.isVertical
          ? `translate3d(0, ${
              totalShiftedItems * (100 / this.d_numVisible)
            }%, 0)`
          : `translate3d(${
              totalShiftedItems * (100 / this.d_numVisible)
            }%, 0, 0)`;
  ...
  navForward(e: TouchEvent | MouseEvent) {
    ...
    let activeIndex =
      this.circular && (<any[]>this.value).length - 1 === this._activeIndex
        ? 0
        : nextItemIndex;
    this.onActiveIndexChange.emit(activeIndex);
  ...
  navBackward(e: TouchEvent | MouseEvent) {
    ...
    if (
      this.d_numVisible - diff - 1 > this.getMedianItemIndex() &&
    ...
    let activeIndex =
      this.circular && this._activeIndex === 0
        ? (<any[]>this.value).length - 1
        : prevItemIndex;
    this.onActiveIndexChange.emit(activeIndex);
  ...
  step(dir: number) {
    ...
    if (
      dir < 0 &&
      -1 * totalShiftedItems + this.d_numVisible >
        (<any[]>this.value).length - 1
    ) {
      totalShiftedItems = this.d_numVisible - (<any[]>this.value).length;
  ...
  getTotalPageNumber() {
    return (<any[]>this.value).length > this.d_numVisible
      ? (<any[]>this.value).length - this.d_numVisible + 1
      : 0;
  }
  ...
  onTouchEnd(e: TouchEvent) {
    ...
    if (this.isVertical) {
      this.changePageOnTouch(
        e,
        touchobj.pageY - (<{ x: number; y: number }>this.startPos).y
      );
    } else {
      this.changePageOnTouch(
        e,
        touchobj.pageX - (<{ x: number; y: number }>this.startPos).x
      );
    }
  }
  ...
  isNavBackwardDisabled() {
    return (
      (!this.circular && this._activeIndex === 0) ||
      (<any[]>this.value).length <= this.d_numVisible
    );
  }
  ...
  isNavForwardDisabled() {
    return (
      (!this.circular &&
        this._activeIndex === (<any[]>this.value).length - 1) ||
      (<any[]>this.value).length <= this.d_numVisible
    );
  }
  ...
  isItemActive(index: number) {
    return (
      this.firstItemAciveIndex() <= index && this.lastItemActiveIndex() >= index
    );
  }
