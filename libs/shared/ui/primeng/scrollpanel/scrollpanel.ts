...
@Component({
  ...
  template: `
    <div
      #container
      [ngClass]="'p-scrollpanel p-component'"
      [ngStyle]="style"
      [class]="styleClass"
    >
...
export class ScrollPanel implements AfterViewInit, AfterContentInit, OnDestroy {
  ...
  calculateContainerHeight() {
    ...
    let containerStyles: { [klass: string]: any } =
        window.getComputedStyle(container),
      xBarStyles = window.getComputedStyle(xBar),
      pureContainerHeight =
        DomHandler.getHeight(container) - parseInt(xBarStyles['height'], 10);
  ...
  moveBar() {
    ...
    this.requestAnimationFrame(() => {
      if ((this.scrollXRatio as number) >= 1) {
        ...
        const xBarLeft =
          (content.scrollLeft * (100 - xBarWidth)) / (totalWidth - ownWidth);
        xBar.style.cssText =
          'width:' +
          xBarWidth +
          '%; left:' +
      ...
      if ((this.scrollYRatio as number) >= 1) {
        ...
        const yBarTop =
          (content.scrollTop * (100 - yBarHeight)) / (totalHeight - ownHeight);
        yBar.style.cssText =
          'height:' +
          yBarHeight +
          '%; top: calc(' +
  ...
  onMouseMoveForXBar(e: MouseEvent) {
    ...
    this.requestAnimationFrame(() => {
      (this.contentViewChild as ElementRef).nativeElement.scrollLeft +=
        deltaX / (this.scrollXRatio as number);
    });
  }
  ...
  onMouseMoveForYBar(e: MouseEvent) {
    ...
    this.requestAnimationFrame(() => {
      (this.contentViewChild as ElementRef).nativeElement.scrollTop +=
        deltaY / (this.scrollYRatio as number);
    });
  }
  /**
