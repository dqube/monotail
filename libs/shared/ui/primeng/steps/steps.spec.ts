...
describe('Steps', () => {
  ...
  it('should change style & styleClass', () => {
    ...
    const stepsEl = fixture.debugElement.children[0].query(
      By.css('div')
    ).nativeElement;
  ...
  it('should show the step number', () => {
    ...
    for (let x = 0; x < testComponent.items.length; x++) {
      expect(
        items[x].query(By.css('.p-steps-number')).nativeElement.textContent
      ).toEqual((x + 1).toString());
    }
  });
  ...
  it('should activate first item', () => {
    ...
    for (let x = 0; x < testComponent.items.length; x++) {
      if (x == 0)
        expect(items[x].nativeElement.className).toContain(
