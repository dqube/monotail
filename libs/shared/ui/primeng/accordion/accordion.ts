    ...
    >
      ...
      >
        ...
        >
          ...
          <ng-template
            *ngTemplateOutlet="iconTemplate; context: { $implicit: selected }"
          ></ng-template>
...
export class AccordionTab implements AfterContentInit, OnDestroy {
  ...
  get hasHeaderFacet(): boolean {
    return (
      (this.headerFacet as QueryList<Header>) &&
      (this.headerFacet as QueryList<Header>).length > 0
    );
  }
...
export class Accordion implements BlockableUI, AfterContentInit, OnDestroy {
   ...
   * @group Emits
   */
  @Output() activeIndexChange: EventEmitter<number | number[] | null> =
    new EventEmitter();
  ...
  ngAfterContentInit() {
    ...
    this.tabListSubscription = (
      this.tabList as QueryList<AccordionTab>
    ).changes.subscribe((_) => {
      this.initTabs();
    });
  }
    ...
    if (this.tabs && this.tabs.length && this._activeIndex != null) {
      for (let i = 0; i < this.tabs.length; i++) {
        let selected = this.multiple
          ? (this._activeIndex as number[]).includes(i)
          : i === this._activeIndex;
