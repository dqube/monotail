...
@Component({
  template: `<p-terminal
    welcomeMessage="Welcome to PrimeNG"
    prompt="@primeng $"
  ></p-terminal>`,
})
class TestTerminalComponent {
  ...
  constructor(private terminalService: TerminalService) {
    this.terminalService.commandHandler.subscribe((command) => {
      let response =
        command === 'd' ? 'Command succeed' : 'Unknown command: ' + command;
...
describe('Terminal', () => {
  ...
  it('should handle command', () => {
    ...
    const responseEl = fixture.debugElement
      .query(By.css('.p-terminal-content'))
      .queryAll(By.css('div'))[1];
