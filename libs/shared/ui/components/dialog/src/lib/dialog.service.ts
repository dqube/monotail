...
export class DialogService {
  ...
  open(
    ...
    config: Partial<DialogConfig<any>> = {}
  ): DialogRef {
  ...
  private attach({
    ...
    view,
    attachToApp,
  }: AttachOptions): DialogRef<any, any, any> {
  ...
  private throwIfIDAlreadyExists(id: string) {
    if (this.dialogs.find((dialog) => dialog.id === id)) {
      throw new Error(
        `Please, ID must be unique, but there is already a dialog created with this ID: ${id}`
      );
    }
  }
}
...
function throwMustBeAComponentOrATemplateRef(value: unknown): never {
  throw new TypeError(
    `Dialog must receive a Component or a TemplateRef, but this has been passed instead: ${value}`
  );
}
